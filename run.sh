...
EOF
    ...
    all)
      find "$SERVICES_DIR" -maxdepth 2 -name docker-compose.override.yml \
...
TMPFILE="$REPO_ROOT/.run-orchestrator.compose.yml"
{
  ...
  echo "include:"
  echo "  - path: infra/docker-compose.yml"
  skipped=""
  for name in $SERVICES; do
    override="$SERVICES_DIR/$name/docker-compose.override.yml"
    if [ -f "$override" ]; then
      echo "  - path: services/$name/docker-compose.override.yml"
    else
