...
#   ./run.sh tooling                     # infra + observability tools (akhq :8080)
...
KNOWN_STACKS="infra tooling core clinical billing insurance devices comms erp-bridge all"
...
list_stacks() {
  ...
  for name in $KNOWN_STACKS; do
    ...
    elif [ "$name" = "tooling" ]; then
      printf "  %-13s  (infra + observability tools — akhq on :8080)\n" "$name"
    elif [ "$name" = "all" ]; then
...
# ── Resolve services ─────────────────────────────────────────────────
# core is implicitly added to any non-infra, non-tooling, non-all stack.

want_core=0
want_tooling=0
for s in $STACK_NAMES; do
  case "$s" in
    tooling) want_tooling=1 ;;
    infra|all|core) ;;
    *) want_core=1 ;;
  esac
done
...
TMPFILE="$REPO_ROOT/.run-orchestrator.compose.yml"
{
  ...
  if [ "$want_tooling" -eq 1 ]; then
    echo "  - path: $INFRA_DIR/tooling/docker-compose.yml"
  fi
  skipped=""
